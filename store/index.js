...
const MyContextControllerProvider = ({children}) => {
  ...
  const fetchStaffMembers = async () => {
    try {
      const staffQuerySnapshot = await USERS.where(
        'email',
        '>=',
        'STAFF',
      ).get();

      // Lọc chỉ lấy những nhân viên có email bắt đầu bằng "STAFF"
      const staffList = staffQuerySnapshot.docs
        .map(doc => ({id: doc.id, ...doc.data()}))
        .filter(staff => staff.email.startsWith('STAFF')); // Lọc danh sách nhân viên
