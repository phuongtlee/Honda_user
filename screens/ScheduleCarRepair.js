...
export default function ScheduleCarRepair({navigation}) {
  ...
  const scrollY = useRef(new Animated.Value(0)).current; // Create a ref for scroll position
  ...
  const handleScroll = event => {
    ...
    if (currentOffset > 0) {
      setButtonVisible(false); // Hide button when scrolling down
    } else {
      setButtonVisible(true); // Show button when at the top
    }
  };
  ...
  return (
    <View style={styles.container}>
      <ScrollView
        ...
        scrollEventThrottle={16} // Throttle for smoother performance
      >
        {loading ? (
...
const styles = StyleSheet.create({
  ...
  scrollContainer: {
    paddingBottom: 100, // To ensure scroll view has enough space
  },
  label: {
    fontSize: 16,
    marginVertical: 10,
  ...
  scheduleButton: {
    ...
    elevation: 3,
    minWidth: 150, // Thêm chiều rộng tối thiểu cho nút
