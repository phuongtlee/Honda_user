...
const AddVehicle = () => {
  ...
  const handleSubmit = async () => {
    ...
    await addVehicle(dispatch, vehicleData); // Đảm bảo addVehicle trả về một Promise
  ...
  const selectImage = () => {
    ImageCropPicker.openPicker({
      mediaType: 'photo',
      includeBase64: true, // Optional: include base64 string
    })
      .then(image => {
        setImageUri(image.path); // Lưu trữ URI của ảnh đã chọn
      })
      .catch(error => {
