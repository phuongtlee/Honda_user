...
const BillingScreen = ({route, navigation}) => {
  ...
  const handleSaveBilling = async () => {
    ...
    try {
      ...
      // Update repair schedule status to "Đã hoàn thành"
      await firestore()
        .collection('repairSchedules')
        .where('carName', '==', carName) // Match the car name to find the repair schedule
        .where('status', '==', 'Chưa hoàn thành') // Ensure it's the pending status
        .get()
        .then(snapshot => {
      ...
      // Update maintenance date in the vehicle collection
