...
const DetailHistory = ({route, navigation}) => {
  ...
  const [currentSchedule, setCurrentSchedule] = useState(schedule); // State to hold the current schedule data
  ...
  // Listener to update the schedule when returning from UpdateSchedule
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      const fetchUpdatedSchedule = async () => {
        try {
          const updatedDoc = await firestore()
            .collection('schedules') // Make sure to replace with the correct collection name
            .doc(currentSchedule.id) // Assuming id is the identifier for the schedule
            .get();
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <Text style={styles.info}>
          Ngày: {new Date(currentSchedule.date).toLocaleDateString()}
        </Text>
        <Text style={styles.info}>
          Giờ:{' '}
          {new Date(currentSchedule.date).toLocaleTimeString([], {
            hour: '2-digit',
            minute: '2-digit',
          })}
        </Text>
