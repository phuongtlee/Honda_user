...
const DetailHistory = ({route, navigation}) => {
  ...
  const [reviewSubmitted, setReviewSubmitted] = useState(false); // Flag for checking if review is submitted
  ...
  useEffect(() => {
    const checkUserRole = async () => {
      ...
      if (email && email.includes('STAFF')) {
        setIsStaff(true); // Mark the user as staff if email contains "STAFF"
      }
    };
    checkUserRole();
  }, []);
  ...
  const checkIfReviewExists = async () => {
    try {
      ...
      if (!reviewsQuery.empty) {
        // If a review already exists, prevent new review submission
  ...
  const handleSaveReview = async () => {
    ...
    try {
      ...
      setReviewSubmitted(true); // Mark as review submitted
  ...
  const handleStarPress = star => {
    if (!reviewSubmitted) {
      setRating(star); // Set the rating when a star is pressed
    }
  };
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        {/* Review Section: Only show if status is "Đã hoàn thành" and user is not staff */}
